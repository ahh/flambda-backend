    ...
    | Const jkind -> 
      ...
      let is_value = Jkind.Type.Const.equal jkind value_jkind
        ...
        || (not Language_extension.(is_at_least Layouts Alpha)
            && Jkind.Type.Const.equal jkind value_or_null_jkind)
      in
...
let tree_of_type_decl id decl =
  ...
  let type_defined decl =
    let abstr =
      match decl.type_kind with
        Type_abstract _ ->
          decl.type_manifest = None || decl.type_private = Type_private
      | Type_record _ ->
          decl.type_private = Type_private
  ...
  let ty, priv, unboxed =
    ...
    | Type_abstract _ ->
        ...
        | None -> (Otyp_abstract, Type_public, false)
        | Some ty ->
